                    ...
                    if (response !== null) {
                        ...
                        for (let i=0;i<response.length;i++) {
                            ...
                            if(categoryIds.includes(response[i].category1_id) || categoryIds.includes(response[i].category2_id)) {
                                if(!supplierIds.includes(response[i].supplier_id)) {
                                    supplierIds.push(response[i].supplier_id);
                                }
                            }
                        }
                        console.log("supplierIds",supplierIds);

                        getSuppliers(encryptStorage.getItem("company-id")).then((supplierResponse) => {
                            if (supplierResponse !== null) {
                                console.log("Suppliers response", supplierResponse);
                                let supp = [];
                                for(let i=0;i<supplierResponse.length;i++) {
                                    if(supplierIds.includes(supplierResponse[i].id)) {
                                        supp.push(supplierResponse[i]);
                                    }
                                }
                                setData(supp);
